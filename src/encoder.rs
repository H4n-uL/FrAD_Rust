...
use crate::{
    common::{
        check_overwrite, format_si, format_speed, format_time, get_file_stem, read_exact,
        write_safe, PIPEIN, PIPEOUT,
    },
    tools::{cli::CliParams, process::ProcessInfo},
};
use libfrad::{head, profiles::LOSSLESS, Encoder};
...
pub fn set_files(
    ...
    if PIPEIN.contains(&rfile.as_str()) {
        rpipe = true;
    } else if !Path::new(&rfile).exists() {
        eprintln!("Input file doesn't exist");
        exit(1);
    }
    if PIPEOUT.contains(&wfile.as_str()) {
        wpipe = true;
    ...
    if wfile.is_empty() {
        wfile = get_file_stem(&rfile);
    }
    if !(wfile.ends_with(".frad")
        || wfile.ends_with(".dsin")
        || wfile.ends_with(".fra")
        || wfile.ends_with(".dsn"))
    {
        if LOSSLESS.contains(&profile) {
            if wfile.len() <= 8 && wfile.is_ascii() {
                wfile = format!("{}.fra", wfile);
            } else {
                wfile = format!("{}.frad", wfile);
            }
        } else if wfile.len() <= 8 && wfile.is_ascii() {
            wfile = format!("{}.dsn", wfile);
        } else {
            wfile = format!("{}.dsin", wfile);
        }
    }

    check_overwrite(&wfile, overwrite);

    let readfile: Box<dyn Read> = if !rpipe {
        Box::new(File::open(rfile).unwrap())
    } else {
        Box::new(std::io::stdin())
    };
    let writefile: Box<dyn Write> = if !wpipe {
        Box::new(File::create(wfile).unwrap())
    } else {
...
pub fn logging_encode(loglevel: u8, log: &ProcessInfo, linefeed: bool) {
    ...
    eprint!(
        "size={}B time={} bitrate={}bit/s speed={}x    \r",
        format_si(log.get_total_size() as f64),
        format_time(log.get_duration()),
        format_si(log.get_bitrate()),
        format_speed(log.get_speed())
    );
    if linefeed {
        eprintln!();
    }
...
pub fn encode(input: String, params: CliParams) {
    ...
    if params.srate == 0 {
        eprintln!("Sample rate should be set except zero");
        exit(1);
    }
    ...
    let (mut readfile, mut writefile) =
        set_files(input, params.output, params.profile, params.overwrite);

    let mut image = Vec::new();
    if !params.image_path.is_empty() {
        match File::open(&params.image_path) {
            Ok(mut imgfile) => {
                imgfile.read_to_end(&mut image).unwrap();
            }
            Err(_) => {
