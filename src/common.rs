...
pub fn write_safe(wfile: &mut Box<dyn Write>, buf: &[u8]) {
    wfile.write_all(buf).unwrap_or_else(|err| match err.kind() {
...
pub fn get_file_stem(file_path: &str) -> String {
    if PIPEIN.contains(&file_path) || PIPEOUT.contains(&file_path) {
        return "pipe".to_string();
    }
    return Path::new(file_path)
        .file_stem()
        .unwrap_or(std::ffi::OsStr::new("pipe"))
        .to_str()
        .unwrap()
        .to_string();
}
...
pub fn format_time(mut n: f64) -> String {
    if n < 0.0 {
        return format!("-{}", format_time(-n));
    }

    let julian = (n / 31557600.0) as u16;
    n = n % 31557600.0;
    let days = (n / 86400.0) as u16;
    n = n % 86400.0;
    let hours = (n / 3600.0) as u8;
    n = n % 3600.0;
    let minutes = (n / 60.0) as u8;
    n = n % 60.0;
    ...
    return {
        if julian > 0 {
            format!(
                "J{}.{:03}:{:02}:{:02}:{:06.3}",
                julian, days, hours, minutes, n
            )
        } else if days > 0 {
            format!("{}:{:02}:{:02}:{:06.3}", days, hours, minutes, n)
        } else if hours > 0 {
            format!("{}:{:02}:{:06.3}", hours, minutes, n)
        } else if minutes > 0 {
            format!("{}:{:06.3}", minutes, n)
        } else if n >= 1.0 {
            format!("{:.3} s", n)
        } else if n >= 0.001 {
            format!("{:.3} ms", n * 1000.0)
        } else if n >= 0.000001 {
            format!("{:.3} µs", n * 1000000.0)
        } else if n > 0.0 {
            format!("{:.3} ns", n * 1000000000.0)
        } else {
            "0".to_string()
        }
    };
...
pub fn format_si(n: f64) -> String {
    ...
    format!(
        "{:.3} {}",
        n as f64 / 1000.0f64.powi(exp as i32),
        UNITS[exp]
    )
}
...
pub fn check_overwrite(writefile: &str, overwrite: bool) {
    if Path::new(writefile).exists() && !overwrite {
        if std::io::stdin().is_terminal() {
            ...
            eprintln!("Output file already exists, please provide --force(-y) flag to overwrite.");
            exit(0);
        }
    }
}
